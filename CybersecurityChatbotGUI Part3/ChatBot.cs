...
namespace CybersecurityChatbotGUI
{
    class ChatBot
    {
        ...
        {
            "I can hear that you're worried. That's completely okay! You're already doing the right thing by asking questions. Try asking me about phishing, malware, or passwords.",
            "It's normal to feel anxious about online safety. Take a breath — I'm here to help.",
            "Feeling unsafe online is very common. Ask me about passwords or privacy to get started!"
        };

        private List<string> _frustratedNoKeyword = new List<string>
        {
            "I completely understand your frustration — cybersecurity can feel overwhelming at first. What topic is confusing you?",
            "No need to feel frustrated! Ask me about any topic from the menu on the left.",
            "It's okay to feel confused. Tell me which topic you want to understand better."
        };

        private List<string> _curiousNoKeyword = new List<string>
        {
            "I love your curiosity! Ask me about passwords, phishing, malware, privacy, scams, firewall, encryption, 2FA, backup, or VPN!",
            "Great that you want to learn more! Type any cybersecurity topic you're curious about.",
            "Curiosity is the first step to staying safe online! Ask me anything."
        };
        ...
        private string ExtractName(string input)
        {
            ...
            string[] greetings = { "good morning", "good evening", "good afternoon", "greetings", "hello", "hey", "hi" };
            foreach (string g in greetings)
            {
                if (lower.StartsWith(g))
                {
                    lower = lower.Substring(g.Length).Trim();
                    break;
                }
            }
            ...
            string[] phrases = { "my name is", "i am", "i'm", "its", "it's", "they call me", "you can call me", "the name is", "name is" };
            foreach (string phrase in phrases)
            {
                if (lower.StartsWith(phrase))
                {
                    lower = lower.Substring(phrase.Length).Trim();
                    break;
                }
            }
        ...
        public string ProcessInput(string input)
        {
            ...
            if (_awaitingName)
            {
                ...
                else
                {
                    ...
                    return "Nice to meet you, " + _memory.UserName + "! 🛡️\n" +
                           ...
                           "💡 New in Part 3: Type 'add task', 'start quiz', or 'show activity log'!";
                }
            }

            // STEP 1B: Awaiting reminder for a pending task
            if (_awaitingReminder)
            {
                ...
                {
                    string reminder = trimmed;
                    string result = _taskManager.AddTask(_pendingTaskTitle, _pendingTaskDescription, reminder);
            ...
            // STEP 2: NLP — Show full activity log
            ...
            // STEP 4: NLP — Start quiz
            if (lower.Contains("start quiz") || lower.Contains("take quiz") ||
                lower.Contains("quiz me") || lower.Contains("test my knowledge") ||
                lower.Contains("play the game") || lower.Contains("start the quiz") ||
                lower.Contains("begin quiz") || lower.Contains("i want to take a quiz"))
            {
